function refreshOverviewFromData(){
  ...
  const feet=[jobs.length?'含目标岗位 '+jobs.filter(j=>j.status==='想投递').length+' 个':'等待添加岗位',totalTasks?'待处理 '+totalTasks+' 项':'暂无待办',next?'最近面试 '+next.date:'还没有面试安排',offerCount?'继续保持':'还在积累机会'];
